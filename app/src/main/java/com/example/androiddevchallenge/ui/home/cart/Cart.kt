...
package com.example.androiddevchallenge.ui.home.cart
...
import com.example.androiddevchallenge.model.OrderLine
...
import com.example.androiddevchallenge.ui.components.QuantitySelector
...
import com.example.androiddevchallenge.ui.home.DestinationBar
...
import com.example.androiddevchallenge.ui.utils.formatPrice
...
fun Cart(
    ...
    val inspiredByCart = remember { PetRepo.getInspiredByCart() }
    Cart(
        orderLines = orderLines,
        removeSnack = viewModel::removePet,
...
private fun CartContent(
    ...
    LazyColumn(modifier) {
        ...
        item {
            SummaryItem(
                subtotal = orderLines.map { it.pet.price * it.count }.sum(),
                shippingCosts = 369
            )
        }
        item {
            SnackCollection(
                petCollection = inspiredByCart,
                onSnackClick = onSnackClick,
                highlight = false
            )
...
fun CartPreview() {
    JetsnackTheme {
        Cart(
            orderLines = PetRepo.getCart(),
            removeSnack = {},
            increaseItemCount = {},
            decreaseItemCount = {},
            inspiredByCart = PetRepo.getInspiredByCart(),
            onSnackClick = {}
        )
    }
}
