...
class CartViewModel : ViewModel() {
    private val _orderLines: MutableStateFlow<List<OrderLine>> =
        MutableStateFlow(PetRepo.getCart())
    val orderLines: StateFlow<List<OrderLine>> get() = _orderLines

    fun removePet(petId: Long) {
        _orderLines.value = _orderLines.value.filter { it.pet.id != petId }
    }

    fun increaseSnackCount(snackId: Long) {
        val currentCount = _orderLines.value.first { it.pet.id == snackId }.count
